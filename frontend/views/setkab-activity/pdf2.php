...
<section class="sheet padding-25mm">
  ...
  <article style="margin-top: -20px;">
    ...
    <table width="100%" class="rekomendasi" style="font-family: cambria;">
      ...
      <tr>
        <td colspan="7" class="uraian"><?php echo $resultIntegritasUraian; ?></td>
      </tr>
      </tbody>
    </table>
    ...
    <table width="100%" class="rekomendasi" style="font-family: cambria;">
      ...
      <tr>
        <td colspan="7" class="uraian"><?php echo $resultPerekatBangsaUraian; ?></td>
      </tr>
      </tbody>
    </table>
    <?php echo $footerReport;?>
...
<div style="margin-bottom:10px;text-align:justify;text-justify:inter-word; font-size: 18px; font-family: cambria;">
  <p>';
  echo '<ul>';
  $kekuatan_items = getListitem(HtmlPurifier::process($activityModel->kekuatan));
  $total_len = 0;
  $break_1 = 0;
...
echo '<ul>';
$kelemahan_items = getListitem(HtmlPurifier::process($activityModel->kelemahan));

foreach ($kelemahan_items as $kelemahan_item){
  if (($total_len > 1500) && ($break_1 == 0))
{
  echo $pagebreakResume;
  $break_1 = 1;
  echo '<ul>';
...
$pagebreak_saran = $footerReport.'</b></p></article>
  ...
  $dom = new DOMDocument();
  $uraian_saran = $activityModel->saran;
  //$uraian_saran = str_replace('“','RENO', $saran2);
  $dom->loadHTML('<?xml encoding="utf-8" ?>' . HtmlPurifier::process($uraian_saran));
...
/*
echo '<ul>';
  $saran_items = getListitem($activityModel->saran);
  $total_len2 = 0;
  $break_2 = 0;
  foreach ($saran_items as $saran_item){

    if (($total_len2 > 2200) && ($break_2 == 0))
    {
      echo $pagebreak_saran;
      $break_2 = 1;
    }
    ...
    $total_len2 = $total_len2 + strlen($saran_item);
    echo '<li>' . $saran_item. '</li>';
  }
  echo '</ul>';
  */
/*
  echo $header_print_saran;
echo '<ul>';
*/


//echo $uraian_saran;
//echo htmlspecialchars($activityModel->saran);
