...
<?php
    ...
    $PArp = 0;
    $PArp += $model->psikogram_kemampuananalisa;
    $PArp += $model->psikogram_empati;
    //$PArp += $model->psikogram_fleksibilitasberpikir;
    $PArp += $model->psikogram_kemampuanumum;
...
])->label('<h3><strong>(Kekuatan) Hal-hal positif yang menunjang tampilnya kinerja yang optimal:</strong></h3>');

$dom = new DOMDocument;
$li_count = 0;
$word_count = 0;

if (!empty($model->kekuatan)) {
$dom->loadHTML(HtmlPurifier::process($model->kekuatan));


    foreach($dom->getElementsByTagName('li') as $li) {
      //  echo '<br/>' . $li->textContent . ' : ';
        $add = 0;
        $add = str_word_count(strip_tags($li->textContent));
        //echo $add;
        $li_count = $li_count + $add;
    }
